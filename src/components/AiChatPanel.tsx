...
function renderMarkdown(text: string, isDark: boolean) {
  ...
  const codeColor = isDark ? '#111111' : '#111111';
...
function TypingIndicator({ isDark }: { isDark: boolean }) {
  const dotColor = isDark ? '#111111' : '#111111';
  return (
...
export const AiChatPanel = React.memo(function AiChatPanel({
  ...
  const accentBg = isDark ? 'rgba(38,51,116,0.18)' : 'rgba(0,0,0,0.03)';
  const userBg = isDark ? 'rgba(255,211,50,0.16)' : 'rgba(0,0,0,0.03)';
  const accentText = isDark ? '#111111' : '#111111';
      ...
      >
        ...
        <Pressable
          ...
          style={{
            ...
            height: 32,
            borderRadius: 16,
            backgroundColor: isThinking ? (isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)') : '#111111',
            alignItems: 'center',
    ...
    >
      {!isUser && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 2 }}>
          ...
          <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 0.8, color: isDark ? '#111111' : '#111111' }}>
            Bebo
          </Text>
        </View>
      )}
